...
exports.cancelBooking = async (req, res) => {
    try {
        ...
        if (booking.userId.toString() !== req.user.id && req.user.role !== 'admin') {
            return res.status(403).json({ message: 'Not authorized' });  //authorization : only the user who created booking can cancel it
        }
        ...
        if (wasConfirmed) {
            ...
            if (event) {
                event.availableSeats += 1;   // restore the seat after cancelllation
                await event.save();
            }
        }
