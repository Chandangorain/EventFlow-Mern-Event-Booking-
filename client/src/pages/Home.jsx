...
const Home=()=>{
    ...
    {/* User types → search state updates → useEffect triggers → wait 400ms → fetchEvents() → API call → setEvents → UI updates */}
     const [events, setEvents] = useState([]);  /*store data */
    ...
    const [loading, setLoading] = useState(true);
     ...
     useEffect(() => {
        const timeoutId = setTimeout(() => {
            fetchEvents();
        }, 400); // 400ms debounce : wait 400 ms before calling apis 
